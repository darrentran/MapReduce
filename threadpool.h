...
typedef struct {
    /**
     * queue - queue used to hold work items
     **/
...
typedef struct {
     ...
     * work_mutex - Mutex for threads in pool
     * work_available_cond - Condition for threads to be notified if there are still work items in the queue
     * stop_running - Boolean to check if thread should stop running
     * live_threads - Number of threads that are still alive
     **/
